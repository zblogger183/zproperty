...
export async function generateMetadata({ params, searchParams }: PageParams): Promise<Metadata> {
  ...
  return buildSearchMetadata({ purpose: "rent", citySlug: city, searchParams: sp });
}

export default async function CityListingsRentPage({ params, searchParams }: PageParams) {
  const { city } = await params;
  const sp = await searchParams;
  return <SearchResultsPage purpose="rent" citySlug={city} searchParams={sp} />;
}
